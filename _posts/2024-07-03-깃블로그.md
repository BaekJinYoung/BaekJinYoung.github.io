...
윈도우는 공식적으로 지킬이 지원되는 OS가 아니기 때문에 Ruby 설치가 필요하다.[1]: https://jekyllrb.com/docs/installation/windows/#installing-ruby-and-jekyll
...
install for me only(나만을 위한 설치)는 현재 프로그램을 설치하는 사용자만 해당 프로그램을 사용할 수 있습니다.
이 선택지를 클릭하면 Ruby가 'C:\Users\[사용자 이름]\AppData\Local'와 같이 해당 사용자 폴더에 설치됩니다.
이 선택지는 관리자 권한이 필요하지 않습니다.

install for all users(모든 사용자에게 설치)를 선택하면 이 컴퓨터를 사용하는 모든 사용자가 해당 프로그램을 사용할 수 있습니다.
Ruby가 'C:\Program Files' 또는 'C:\Program Files (x86)' 와 같은 공용 폴더에 설치됩니다.
이 선택지는 관리자 권한이 필요하다.

install for me only를 선택한다.
...
I accept the License는 소프트웨어 라이선스 사용권 계약서의 조건에 동의하겠다는 뜻이다.
동의해야 다음으로 넘어갈 수 있다.
...
Add Ruby executables to your PATH를 선택하면 루비와 관련된 실행 파일이 운영 체제의 PATH 환경 변수에 추가된다.
이를 통해 명령 프롬프트나 터미널에서 루비 명령어를 전역적으로(어디서든지) 사용할 수 있게 된다.
루비 실행 파일에는 루비 인터프리터(ruby)와 패키지 관리자(gem) 등이 포함되어 있다.

Associate .rb and .rbw files with this Ruby installation을 선택하면 .rb 및 .rbw 같은 특정 파일 형식을 현재 설치 중인 루비 인터프리터와 연결한다.
이를 통해 해당 파일들을 더블클릭하거나 명령 프롬프트에서 실행할 때 자동으로 루비 인터프리터를 사용하여 실행할 수 있게 된다.

.rb 파일은 Ruby 스크립트 파일이다. Ruby 코드가 포함된 텍스트 파일이다.
.rbw 파일은 일반적으로 GUI 애플리케이션을 작성할 때 사용된다. 콘솔 창을 열지 않고 실행된다.
...
Select Components는 사용자가 설치할 구성 요소를 선택하는 단계이다.

Ruby RI and HTML documentation

RI (Ruby Index) Documentation
RI는 명령줄에서 사용할 수 있는 문서 도구이다.
루비 표준 라이브러리 및 설치된 Gem에 대한 문서를 제공한다.
Ri를 설치하면 'ri' 명령어를 사용해 터미널에서 문서를 조회할 수 있다.

HTML documentation
웹 브라우저에서 볼 수 있는 형식으로 제공되는 문서이다.
루비와 관련된 모든 내용을 하이퍼링크로 연결된 웹 페이지로 제공한다.

MSYS2 development toolchain
MSYS2는 Windows에서 Unix 스타일의 개발 환경을 제공하는 소프트웨어 배포판이다.
Unix나 Linux에서 사용하는 셸과 유틸리티들을 Windows에서도 사용할 수 있게 합니다.

둘 모두 선택하고 install 버튼을 클릭해 다음 단계로 넘어간다.
...
run 'rdk install' to set up MSYS2 and development toolchain. MSYS2 is required to install gems with C extentions.
MSYS2와 개발 도구 체인을 설정하기 위해 'rdk install'을 실행하세요. C 확장이 포함된 Gem을 설치하려면 MSYS2가 필요합니다. 라는 뜻이다.
선택한 채로 Finish 버튼을 클릭해 설치를 마치면 자동으로 프롬포트 창이 열리고, MSYS2 설치 화면이 나온다.
...
Windows에서 널리 사용되는 GNU 개발 도구들을 포함하는 프로젝트이다.

엔터키를 눌러 MSYS2와 MINGW를 설치한다.
...
설치를 완료하면 You can use 'ridk enable' to activate the MSYS2 tools on the command prompt. 라는 문장이 뜬다.
엔터키를 누르면 프롬프트 창이 꺼진다.
...
윈도우 검색창에 루비를 검색하고 Start Command Prompt with Ruby를 클릭한다.
프롬프트 창에서 루비가 실행된다.
...
bundler는 루비 프로젝트에서 사용되는 의존성 관리 도구이다.
...
깃허브 페이지의 우측 상단에 있는 '+' 버튼을 클릭하면 New repository 버튼이 보인다.
클랙해 새 리포지토리를 생성한다.
...
Repository name, 레포지토리 이름은 깃아이디.github.io로 입력한다.
(나는 이미 블로그 레포지토리가 있기 때문에 같은 이름으로 생성이 불가능하다는 경고가 떴다)

Public으로 설정하고, Add a README file에 체크한 뒤 Create repository 버튼을 클릭해 레포지토리를 생성한다.
...
해당 폴더를 열면 블로그가 생성된 것을 확인할 수 있다.
...
생성된 블로그 폴더로 들어가 Gemfile을 수정한다.

1. gem "jekyll"을 주석 처리한다.
2. gem "github-pages" 부분의 주석을 해제하고 버전을 표기한다.

아래 사이트에서 깃허브 페이지에서 참조하는 깃허브 페이지 버전을 확인할 수 있다.
...
수정을 마쳤으면 깃허브에서 블로그 레파지토리로 접속한다.
...
블로그 폴더에 들어있는 모든 파일을 드래그해 여기에 넣는다.
choose your files를 클릭해 업로드할 수도 있다.
...
파일을 끌어온 뒤 Commit changes에서 커밋 메시지를 입력한다. 기본적으로 Add files via upload라고 적혀있다.

커밋 메시지를 입력한 뒤 초록색 Commit changes 버튼을 클릭해 커밋한다.
...
Actions에서 커밋한 내용이 빌드 및 배포되었는지 확인할 수 있다.
진행중일 때에는 동그라미가 노란색이고, 진행이 완료되면 초록색으로 바뀐다.
...
배포가 완료되어 동그라미가 초록색이 되면 Settings -> Pages로 이동한다.

Build and deployment에서 Branch를 main, /(root)로 설정한 뒤 Save 버튼을 클릭해 저장한다.
Visit site 버튼을 클릭하면 블로그에 접속할 수 있다.
...
[1]: https://jekyllrb.com/docs/installation/windows/#installing-ruby-and-jekyll
